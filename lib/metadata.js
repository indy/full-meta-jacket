...
function addFileMetadata(meta, base, folder, f) {
  ...
  if(f === '_vars') {
    ...
    var publishedFilename = getPublishedFilename(f);
    ...
    if(shouldCopyDirectly(publishedFilename)) {
      ...
      // read the file's metadata and add it to *meta* 
      // under the file's published filename
      meta[publishedFilename] = readFileMetadata(base, folder, f);
    }

    // the page's URI should be available as a local
    meta[publishedFilename]._locals.uri = fullUri(folder, publishedFilename);
    meta._files.push(f);
  }
  return meta;
}
