...
function addListsMetadata(meta) {
  ...
  for (var o in meta) {
    if (meta[o]._locals && meta[o]._locals.lists) {
      // hoist these lists up into the local variable 'lists'
      for (var ll in meta[o]._locals.lists) {
        if (lists.hasOwnProperty(ll)) {
          lists[ll] = lists[ll].concat(meta[o]._locals.lists[ll]);
        } else {
          lists[ll] = meta[o]._locals.lists[ll];
        }
      }
    }
