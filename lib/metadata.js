...
function addFileMetadata(meta, base, folder, f) {
  ...
  if(f === '_vars') {
    ...
    if(canContainImbueMetadata(fileMeta.publishedFilename)) {
...
// can this filetype contain imbue style metadata 
function canContainImbueMetadata(filename){
  var ext = utils.pathExtension(filename);
  var allowed = ['imd', 'html', 'txt', 'xml'];

  return allowed.some(function(b) { return b == ext});
}
