...
function scopedMeta(sm, meta, segs) {
  return meta ? scopedMeta(utils.deepMerge(meta._locals, sm),
                           meta[segs[0]],
                           segs.slice(1)) : sm;
...
function addFileMetadata(meta, base, folder, f) {
  ...
  if(f === '_vars') {
    ...
    fh.uri = folder + "/";
    meta = utils.deepMerge(meta, {_locals: fh});
  } else {
    ...
    if((fileMeta._useLess) || (fileMeta._useStylus)) {
      ...
      fileMeta = utils.deepMerge(readFileHeader(pathFilename), fileMeta);
      fileMeta._directCopy = false;
    } else {
...
function mergeTemplateMetadata(metadata) {
  ...
  var fn = function(meta) {
    ...
    if(l && templates[l]) {
      return utils.deepMerge(meta, fn(templates[l].header));
    } else {
      return meta;
    }
  };
...
exports.scopedBuild = function(meta, uriPath, callback) {
  ...
  if(isValidPath(meta, segs)) {
    ...
    if(hasValidTemplate(scoped)) {
      scoped = mergeTemplateMetadata(scoped);
    }
