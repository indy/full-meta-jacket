...
function buildMetadata(base, folder) {
  ...
  utils.mapContents(basef, function(f) {
    path = utils.fileJoin(basef, f);
...
function addFileMetadata(s, base, folder, f) {
  ...
  if(f === '_vars') {
    ...
    s = imbue.mergeMeta(s, {_locals: readFileHeader(basef, f)});
  } else {
...
function addDirectoryMetadata(s, base, folder, f) {
  ...
  if(f === '_posts') {
    s = addPostsDirectoryMetadata(s, base, folder, f);
  } else {
...
function addPostsDirectoryMetadata(s, base, folder, f) {
...
function readFileMetadata(base, folder, f) {
  ...
  var header = readFileHeader(basef, f);
  var basics = {};
