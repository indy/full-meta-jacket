...
function embedInTemplate(bindings, body) {
  ...
  const content = indentLines(body);
...
function bodyIsAllSource(body) {
  // returns true if the body only consists of source code
  const allWhitespace = /^\s*$/;
  const fourSpaces = /^ {4}/;
  const lines = body.split('\n');

  return lines.every(l => l.match(allWhitespace) || l.match(fourSpaces));
}

function wrapAsCode(body) {
  const fourSpaces = /^ {4}/;
  const lines = body.split('\n');
  const res = lines.map(l => l.replace(fourSpaces, '    | '));
...
exports.render = function(bindings, body) {
  ...
  if (bindings._filename && utils.pathExtension(bindings._filename) === 'imd') {
    ...
    // see if the body starts with source code (0 or more whitespace lines
    // followed by 4 spaces and then a character)

    if (bodyIsAllSource(body)) {
      body = wrapAsCode(body.trim());
    } else {
