    ...
    if(scopedMeta._directCopy) {
      ...
      fs.readFile(pathFilename, 'utf8', function(e, data) {
        if(e) {
          ...
          evalContent(pathFilename, scopedMeta, data, callback);
        }
      });
    }
 ...
 */
function evalContent(pathFilename, scopedMeta, content, callback) {
  try {
    if(scopedMeta._useLess === true) {
      evalLessContent(content, callback);
    } else {
      evalImbueContent(scopedMeta, content, callback);
    }
  } catch(e) {
    console.error("evalContent error with " + pathFilename);
    throw e;
  }
}


function evalLessContent(content, callback) {
