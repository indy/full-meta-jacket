...
function render(meta, uriPath, callback) {
  ...
  if(scopedMeta._directCopy) {
    callback(fs.readFileSync(pathFilename));
    return;
  } 

  var content = fs.readFileSync(pathFilename, 'utf8');

  evalContent(scopedMeta, content, callback);
}
...
function evalContent(scopedMeta, content, callback) {
  ...
  if(scopedMeta._filename.split('.').slice(-1)[0] === 'less') {
    evalLessContent(scopedMeta, content, callback);
  } else {
...
function evalLessContent(scopedMeta, content, callback) {
  console.log('rendering less ' + scopedMeta._filename);
