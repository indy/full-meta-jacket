...
exports.renderString = function(meta, uriPath) {
  return renderString(meta, uriPath);
}
...
// **renderPage** renders the resource at uriPath to the 
...
function renderPage(meta, uriPath) {
  ...
  // also provide a reference to the full metadata just in case
  scopedMeta._fullMeta = meta;

  // calculate the full source and destination paths
  var fullPath = meta._base + scopedMeta._filename;
  var fullDest = meta._destination + uriPath;

  // render the page to the filesystem
  page.renderFile(scopedMeta, fullPath, fullDest);
}

// **renderString** renders the resource at uriPath as a string
function renderString(meta, uriPath) {
  ...
  // render the page to the filesystem
  return page.renderString(scopedMeta, fullPath);
}
