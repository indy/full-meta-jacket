  ...
  utils.mapContents(folder, function(f) {
    ...
    if(stats.isFile()) {
      if(f === '_vars') {
        ...
        var meta = imbue.mergeMeta(metadata, {page: metaRelative});
        renderFile(meta, pathfilename, f, destination);
      }
    } else if(stats.isDirectory()) {
      if(f === layout.folder || f === '.git') {
        ...
        if(f === '_posts') {
          ...
          // also merge metaRelative with metaRelative[f]
          folderMeta = imbue.mergeMeta(metaRelative, metaRelative[f]);
        } else {
          subfolder = utils.fileJoin(destination, f);
          folderMeta = metaRelative[f];
        }

        renderFolder(metadata, folderMeta, pathfilename, subfolder);
      }
    } else {
      // raise an error
    }
